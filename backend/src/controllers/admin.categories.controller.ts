...
export const adminGetCategories = asyncHandler(
    async (req: Request, res: Response) => {
        ...
        const pendingCategories = await Category.count({ where: { status: 'pending' } });
...
/**
 * Get category requests (pending)
 * GET /admin/categories/requests
 */
export const getCategoryRequests = asyncHandler(
    async (req: Request, res: Response) => {
        const requests = await Category.findAll({
            where: { status: 'pending' },
            include: [
                {
                    model: User,
                    as: 'creator', // We need to add this association to Category model or use 'createdBy'
 ...
 */
export const approveCategory = asyncHandler(
    async (req: Request, res: Response) => {
        const { id } = req.params;
        ...
        if (category.status === 'active') {
            return sendError(res, ERROR_CODES.BAD_REQUEST, 'Category is already active', 400);
        }

        // Get max sort order if needed
        if (category.sortOrder === 0) {
            const maxSortOrder = await Category.max('sortOrder', {
                where: { parentId: category.parentId || null },
            });
            category.sortOrder = (maxSortOrder as number || 0) + 1;
        }
        ...
        sendSuccess(res, { category }, 'Category approved successfully');
    }
);
 ...
 */
export const rejectCategory = asyncHandler(
    async (req: Request, res: Response) => {
        const { id } = req.params;
        ...
        category.rejectionReason = reason || null;
        await category.save();

        sendSuccess(res, { category }, 'Category rejected successfully');
    }
);

// ... (Copy other admin functions: createCategory, updateCategory, deleteCategory, reorderCategories, toggleCategory)
// For brevity, I will import them from categories.controller.ts in the routes file if possible, 
// OR I should move them here. 
// To avoid duplication and potential issues, I'll move them here.
